    ...
    Self: Clone + Send + Sync + 'static,
{
    ...
    async fn enable_reading(&self) {
        ...
        assert!(self.node().protocols.reading.set(hdl).is_ok(), "the Reading protocol was enabled more than once!");
    }
...
impl<R: Reading> ReadingInternal for R {
    ...
    fn map_codec<T: AsyncRead>(
        ...
        framed.map_decoder(|codec| CountingCodec { codec, node: self.node().clone(), addr, acc: 0 })
    }
}
