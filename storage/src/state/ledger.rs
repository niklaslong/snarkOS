...
#[derive(Debug)]
pub struct LedgerState<N: Network, T: StorageAccess> {
...
impl<N: Network, T: StorageAccess> LedgerState<N, T> {
    ///
    ...
    ///
    pub fn open_reader<S: Storage<Access = T>, P: AsRef<Path>>(path: P) -> Result<(Arc<Self>, Resource)> {
        // Open storage.
...
impl<N: Network, T: StorageReadWrite> LedgerState<N, T> {
    ...
    /// a read-only instance of `LedgerState` may only call immutable methods.
    ///
    pub fn open_writer<S: Storage<Access = T>, P: AsRef<Path>>(path: P) -> Result<Self> {
    ...
    #[doc(hidden)]
    pub fn open_writer_with_increment<S: Storage<Access = T>, P: AsRef<Path>>(path: P, validation_increment: u32) -> Result<Self> {
        // Open storage.
        let context = N::NETWORK_ID;
        let storage = S::open(path, context)?;
...
#[derive(Clone, Debug)]
struct BlockState<N: Network, T: StorageAccess> {
...
impl<N: Network, T: StorageAccess> BlockState<N, T> {
...
struct TransactionState<N: Network, T: StorageAccess> {
...
impl<N: Network, T: StorageAccess> TransactionState<N, T> {
