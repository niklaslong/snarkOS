...
#[derive(Debug)]
pub struct ProverState<N: Network, T: StorageAccess> {
    /// The coinbase records of the prover in storage.
    coinbase: CoinbaseState<N, T>,
}

impl<N: Network, T: StorageAccess> ProverState<N, T> {
...
struct CoinbaseState<N: Network, T: StorageAccess> {
    records: DataMap<N::Commitment, (u32, Record<N>), T>,
}

impl<N: Network, T: StorageAccess> CoinbaseState<N, T> {
